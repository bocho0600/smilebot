...
class COPPELIA_WarehouseRobot(object):
	...
	def GetCameraImage(self):
		...
		res,resolution,self.image=coppelia.simxGetVisionSensorImage(self.clientID,self.cameraHandle,0,coppelia.simx_opmode_buffer)
		
		if res==coppelia.simx_return_ok:
			return resolution, self.image  
		else:
			return None, None
	...
	def Capturing(self):
		resolution, self.frame = self.GetCameraImage()
		self.frame = np.reshape((np.array(self.frame).astype(np.uint8)), (480,640,3))
		self.frame = cv2.flip(cv2.cvtColor(self.frame, cv2.COLOR_BGR2RGB), 0)
		self.frame = cv2.resize(self.frame, (410, 308))
		imgHSV = cv2.cvtColor(self.frame, cv2.COLOR_BGR2HSV)  # Convert to HSV
		robotview = self.frame.copy()  # Preserve the original image
		return self.frame, imgHSV, robotview
